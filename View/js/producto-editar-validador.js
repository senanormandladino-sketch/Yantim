document.addEventListener('DOMContentLoaded', () => {
  ...
  const validarCampo = (input) => {
    ...
    if (input === campos.nombre) {
      const tieneLongitudCorrecta = valor.length >= 3 && valor.length <= 100;
      const iniciaConTresLetras = /^[a-zA-ZáéíóúÁÉÍÓÚñÑ]{3}/.test(valor);

      if (!tieneLongitudCorrecta) {
        esValido = false;
        mensajeError = 'El nombre debe tener entre 3 y 100 caracteres';
      } else if (!iniciaConTresLetras) {
        esValido = false;
        mensajeError = 'Los primeros 3 caracteres deben ser letras';
      }
      ...
      if (!esValido) mensajeError = 'Ingresa un inventario válido (0 o mayor)';
    ...
    } else if (input === campos.foto_texto) {
      esValido = valor.length >= 3;
      if (!esValido) mensajeError = 'Ingresa una ruta o URL válida para la imagen';
      ...
      if (!esValido) mensajeError = 'Solo se permiten archivos de imagen (JPG, PNG, WEBP)';
    }
  ...
  form.addEventListener('submit', (e) => {
    ...
    Object.values(campos).forEach(input => {
      if (input) {
        if (input.hasAttribute('required') || input.value.trim() !== '' || (input.type === 'file' && input.files.length > 0)) {
